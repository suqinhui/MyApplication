...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    private Bitmap bitmapCrazyJava;

    public DBOpenHelper(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
        //获取书籍图片
        bitmapCrazyJava = BitmapFactory.decodeResource(context.getResources(), R.drawable.crazy_java);
    }
...
//        db.execSQL("insert into book_table(book_name,book_english_name,author,introduction,years,image)values('疯狂java实战演义','crazy_java','杨恩雄 麦凯翔','《 疯狂Java实战演义》以15个生动的Java案例，引领读者体验Java开发的乐趣。书中使用Java的Swing技术开发了若干个游戏，从这些游戏中可以了解到，Java一样可以做出优秀的游戏和应用程序。本书知识点丰富，适合有一定Java基础、有意向做Java桌面应用程序或者想了解Java图形界面编程的读者阅读，也可作为Java开发程序员的案例参考书。','2010-6-02','" + imgCrazyJava + "')");
        ...
        db.execSQL("insert into book_chapter(book_english_name,chapter_name,chapter_id,content)values('crazy_java','第6章 仿Windows画图','6','6.1 画图软件概述\n" +
                "我们平时所使用的图形处理工具有PhotoShop、Windows画图工具等，其中PhotoShop是一款非常强大的图形处理工具，Windows画图工具则是一款较为简单的画图工具，功能较为简单，相信经常使用Windows系统的读者都比较熟悉，是一种比较简单与具有代表性的画图工具，虽然功能不够强大，但具有大多图片处理程序所必需的基本功能：铅笔画图、各种数学函数图形、填色、取色、橡皮擦等等功能。\n" +
                "本文将使用Java语言去实现Windows的大部分功能，包括取色、各种数学函数图形、橡皮擦、喷枪、颜色编辑等功能，除了这些绘图功能，还会实现打开图片、保存图片等文件操作功能。画图工具的最终效果如图6.1所示。\n" +
                "\n" +
                "图 6.1 画图\n" +
                "6.2 画图工具原理\n" +
                ...
                "6.3 创建画图工具的各个对象\n" +
                "使用Windows的画图软件，发现在编辑图片的时候，有个相似的过程，首先是用鼠标选择需要使用的工具，然后就在画板中用鼠标进行拖动、点击等动作，画板会显示出相应工具的所产生的效果，所以在这里设计一个命名为Tool的接口，这个接口是所有工具的接口，里面定义了一系列的鼠标动作。实际上在这个画图工具中，所有的工具都必须遵守一定的规范，即使用鼠标进行拖动、点击等动作，当需要定义某些规范的时候，我们可以将这些规范写到一个接口中，那么这个接口所有的实现类都要遵守这个规范，这也是本章将工具作为一个接口的原因。\n" +
                "在本章中，画图软件的主界面使用ImageFrame，该类继承于JFrame，该类会初始化画图软件的各种组件。由于我们有选择打开图片文件的操作，所以会有一个扩展javax.swing.JFileChooser类（为选择文件提供一种简单的窗口选择机制）的ImageFileChooser类，用于处理选择文件时的过滤等操作。\n" +
                ...
                "\uF071static final String ARROW_TOOL，箭头工具类型。\n" +
                "\uF071static final String PENCIL_TOOL，铅笔工具类型。\n" +
                "\uF071static final String BRUSH_TOOL ，刷子工具类型。\n" +
                "\uF071\tstatic final String CUT_TOOL，剪切工具类型。\n" +
                "\uF071\tstatic final String ERASER_TOOL，橡皮擦工具类型。\n" +
                "\uF071\tstatic final String LINE_TOOL ，直线工具类型。\n" +
                "\uF071\tstatic final String RECT_TOOL ，矩形工具类型。\n" +
                "\uF071\tstatic final String POLYGON_TOOL ，多边形工具类型。\n" +
                "\uF071\tstatic final String ROUND_TOOL，椭圆形工具类型。\n" +
                ...
                "从接口中定义的属性与方法可以看出，在接口中只定义工具的类型，还有定义工具鼠标动作的方法，就不再做任何的事情，这些方法由它的实现类去具体实现。在某个对象中需要使用到Tool的实现类时，我们可以使用一个ToolFactor的类来得到具体的某个Tool实现类，ToolFactory返回的都是Tool接口，因此使用者根本不需要关心使用的是哪一个实现类，当代码发生改变的时候，也可以减少代码的修改。换言之，使用者只与ToolFactory耦合。\n" +
                "6.3.2 Tool的实现类AbstractTool\n" +
                "AbstractTool是Tool的实现类，也是一个抽像类，所以并不能被创建，只能被继承。此类实现Tool中定义的所有方法，并扩展了其它方法，让其子类继承或者重写。该类中为其他的工具类提供了大部分的实现，那么它的子类就可以不必再做重复的实现，只关心与本类相关的逻辑，AbstractTool所定义的方法如下：\n" +
                "\uF071AbstractTool( ImageFrame frame )，让子类调用的构造器，以ImagerFrame为参数，用于获取画板的属性。\n" +
                "\uF071AbstractTool( ImageFrame frame, String path )，让子类调用的构造器，以ImagerFrame为参数，path是工具的图标路径。\n" +
                "\uF071Cursor getDefaultCursor()，此方法获取默认鼠标指针的形状。\n" +
                "\uF071void setDefaultCursor( Cursor cursor )，设置鼠标指针指针，以Cursor为参数。\n" +
                "\uF071void setPressX(int x)，设置鼠标按下的x坐标，int类型的x为鼠标的x坐标。\n" +
                "\uF071void setPressY(int y)，设置鼠标按下的y坐标，int类型的y为鼠标的y坐标。\n" +
                "\uF071int getPressX()，返回上次鼠标按下的x坐标。\n" +
                "\uF071int getPressY()，返回上次鼠标按下的y坐标。\n" +
                ...
                "\uF071void createShape( MouseEvent e ,Graphics g )，画图形，通过参数e去获取鼠标的轨迹，并用Graphics类型的对象g去画图形。\n" +
                "\uF071void draw(Graphics g, int x1, int y1, int x2, int y2)，画图形，g是用来画图形的对象，（x1，y1）是起点坐标，（x2，y2）是终点坐标。这个方法是一个空的方法，主要是由其子类实现。\n" +
                ...
                "AbstractTool一共有ArrowTool（箭头）、PencilTool（铅笔）、BrushTool（刷子）、EraserTool（橡皮擦）、LineTool（直线）、RectTool（矩形）、PolygonTool（多边形）、RoundTool（椭圆形）、RoundRectTool（圆矩形）、AtomizerTool（喷墨）、ColorPickedTool（颜色选择）11个子类，这些子类都是根据自己的情况重写AbstractTool的部分或者全部方法。由于在本设计中，想这些类在外表现为Tool接口，不希望被直接实例化，所以此类的构造器私有，并提供一个静态的方法获取Tool类型的此类实现，如下：\n" +
                "\uF071static Tool getInstance( ImageFrame frame )，获取Tool类型的本类实例。\n" +
                "由于我们并不希望外界可以直接使用new关键字来创建这些类的实例，因此在这里使用了单态模式，所有的子类都提供了getInstance的方法来返回本类的实例，并且所有的构造器都是私有的。在下面的章节中，将会讲解如何实现这11个子类。\n" +
                "6.3.4 界面类ImageFrame\n" +
                ...
                "\uF071void setBufferedImage( MyImage bufferedImage)，设置画板图片，MyImage是BufferedImage的一个扩展类。\n" +
                ...
                "\uF071Dimension getScreenSize()，获取Dimension类形screenSize，screenSize主要用于获取画板的高与宽等属性。\n" +
                ...
                "界面类类似于我们MVC模式中的V（视图），该类并不负责处理任何的逻辑，主要负责从界面接收数据，再传递给具体的业务类，让其进行相关的处理。在本章，负责处理画图功能的主要是Tool的实现类。\n" +
                ...
                "\uF071Dimension getScreenSize()，获取屏幕的分辨率。\n" +
                "\uF071repaint( Graphics g, BufferedImage bufferedImage )，刷新界面。\n" +
                "\uF071static Cursor createCursor( String path )，创建鼠标图形。Path是鼠标图形的路径。\n" +
                ...
                "ImageFileChooser类继承了JFleChooser类，JFleChooser是Java提供的一个简单的文件选择机制，我们这里扩展这个类，是为了增加我们自己的文件过滤器。见以下方法：\n" +
                "\uF071String getSuf()，获取文件的后缀名。\n" +
                "\uF071void addFilter()，增加文件过滤器，这里只选择图片类形的文件。\n" +
                ...
                "6.4 主界面实现\n" +
                "在这个软件中，主界面主要由左边的工具栏、下面的颜色选择板、占大部分区域的画图区、菜单等几部分组成，用BorderLayou的排板方式，左边工具栏在BorderLayou.WEST位置，画图区在BorderLayout.CENTER位置，颜色选择面板在BorderLayout.SOUTH位置。先看主界面的初始化：\n" +
                "6.4.1 初始化界面（init()方法）\n" +
                "首先，设置JFrame窗口的标题，接下来初始化画图区域，初始化为白色，然后再获取PENCIL_TOOL(铅笔)类型的Tool，创建各种鼠标监听器，并在监听的执行方法中调用Tool的相应方法，最后获取左边工具栏面板、下面菜单栏面板、菜单，并把这些面板与画图获取加到JFrame中。见以下代码。\n" +
                ...
                "\t}\n" +
                "可以看到，这里有两种鼠标监听器，MouseMotionListener和MouseListener，MouseMotionListener主要是监听鼠标的运动动作，我们实现了它的mouseDragger（鼠标拖动）与mouseMoved（鼠标移动）方法，MouseListener负责监听鼠标的其它动作，我们实现了它的mouseReleased（松开鼠标）、mousePressed（按下鼠标）和mouseClicked（点击鼠标）三个方法。以上代码的黑体部分，这三行代码分别创建菜单、画图工具栏与颜色选择面板，如何创建我们将在6.4.3、6.4.4和6.4.5中详细描述。现在运行画图工具，可以看到效果如图6.3所示。\n" +
                "\n" +
                "图6.3 主界面\n" +
                "6.4.2 获取画板\n" +
                ...
                "\t\t}\n" +
                "\t}\n" +
                "从上面代码可以看到，这个内部类比较简单，只是继承JPanel，并重写JPanel的paint方法，这里需要注意的是，要调用此方法，并不是直接调用paint方法，而是调用ImageService的repaint方法。而获取这个画板就是去创建一个这个画板类的实例，由于我们的画图软件是每次都只编辑一张图片，所以这个创建画板的方法在本类中只被调用一次。首先是new一个DrawSpace实例，再设置这个drwaSpace的大小，并且返回，见以下代码。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\ImageFrame.java\n" +
                " //创建画板\n" +
                ...
                "     -帮助主题\n" +
                "     -关于\n" +
                "由于菜单比较简单，我们把文件、查看、颜色、帮助四个菜单文字放在一个String类型的数组menuArr里面，并迭代这个数组去创建一个JMenu，JMenu就是指菜单。同样，把他们下面的各个菜单项文字也放在一个String类型的二维数组里面，去迭代创建每个JmenuItem（菜单项），每创建完一个，就为它加上一个动作监听器，去监听这个菜单项是否被点击。请看以下代码。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\ImageFrame.java\n" +
                "//创建菜单\n" +
                "public void createMenuBar() {\n" +
                "\t//创建一个JMenuBar放置菜单\n" +
                ...
                "\tthis.setJMenuBar( menuBar );\n" +
                "}\n" +
                "以上代码的粗体部分，是为菜单添加相应的分隔符。在一般的下拉菜单中，可以通过分隔符将菜单划分为多个块。加入菜单后主界面的具体效果如图6.4所示。\n" +
                "\n" +
                "图6.4 加入菜单后的界面\n" +
                "6.4.4 创建画图工具栏\n" +
                "画图工具栏，这里实现的工具有铅笔、刷子、拾色器、喷枪、橡皮擦、直线、多边形、矩形、椭圆形和圆矩形，这里，每个工具我们用一个JButton去代表它，使用JButton的JButton(AbstractActoin action)构造器去创建JButton，用这个构造器创建JButton，可以在AbstractAction中加入按键的图标，以图形的方式创建按钮，另外，我们会有一个AbstractAction的实现类。见以下代码：\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\ImageFrame.java\n" +
                "\tpublic JPanel createToolPanel() {\n" +
                "\t\t//创建一个JPanel\n" +
                "\t\tJPanel panel = new JPanel();\n" +
                ...
                "\t\treturn panel;\n" +
                "\t}\n" +
                "首先，我们创建一个放置这些按钮的JToolBar，JtoolBar是Java Swing提供的一个工具栏类，并设置JToolBar的标题、排列方式（垂直）、是否可以拖动、与边界的距离、还有布局方式，接下来遍历存放这些工具类形的数组，每次都以工具图标的路径、工具名去创建一个ImageAction（AbstractAction）的子类，并以这个ImageAction去创建一个JButton，并把JButton加到JToolBar中。那么，怎么监听这些按钮？在这里，监听的工作就放到继承AbstractAction的ImageAction去实现，见ImageAction中重写的actionPerformed方法，这个方法就是用来执行监听到按钮被点击后的方法。\n" +
                ...
                "\t\t}\n" +
                "\t}\n" +
                "代码中加粗的地方是这个方法的主要业务，首先能过工具的名字name去获取一个Tool接口的实现类实例，如果这个实例不为空，就把它设置为现在正使用的工具。而设置正在使用的颜色这段代码，是用在监听颜色选择时用到，如果color不为空，就把当前的颜色设置为被选择的颜色。加入了画图工具栏后，具体的效果如图6.5所示。\n" +
                "\n" +
                "图6.5 加入画图工具栏\n" +
                "6.4.5 创建颜色选择面板\n" +
                "这是一个简单的颜色选择面板，只有最基本的几种颜色选择（BLACK、BLUE、CYAN、GRAY、GREEN、LIGHT_GRAY、MAGENTA、ORANGE、PINK、RED、WHITE、YELLOW），创建过程与创建工具栏的过程类似，用设置了颜色的按钮去代表这些颜色，首先创建一个JToolBar去放置这些颜色按钮，并设置这个JToolBar的布局方式、标题、是否可以拖动等属性，最后去遍历保存这些颜色类型的数组，每次都新创建一个ImageAction，并以这个ImageAction去创建一个JButton，同时设置这个JButton的颜色，最后加到JToolBar中。见以下代码。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\ImageFrame.java\n" +
                ...
                "\t\treturn panel;\n" +
                "\t}\n" +
                "以上代码主要用于创建颜色选择面板，以上代码的黑体部分，使用了JButton来作为一种颜色的选择按钮，这些颜色选择按键使用了ImageAction作为构造参数，ImageAction在6.4.4中创建，该Action类负责处理用户点击工具栏或者选择颜色时的行为，现在可以运行画图工具，具体的效果如图6.6所示。\n" +
                "\n" +
                "图6.6 主界面\n" +
                "在本小节中，我们使用Swing创建了画图工具的主界面，主界面主要包括菜单、画图工具栏、颜色选择面板与画图区域等。接下来，我们将逐步实现画图里面的功能。\n" +
                "6.5 工具实现\n" +
                "前面说到，这里的工具类设计是有一个定义了工具所有方法的接口Tool，这个接口有一个实现类AbstractTool，该类实现了Tool接口的所有方法，并定义了一些方法供子类重写，Tool接口、AbstractTool类与它的子类关系，如图6.7所示。\n" +
                "\n" +
                "图 6.7 Tool继承关系图\n" +
                "Tool提供mouseDragged、mouseMoved、mouseReleased、mousePressed、mouseClicked五个方法接口，由其实现类实现，接下来先了解所有工具类的父类：AbstractTool。\n" +
                "6.5.1 实现拖动边框改变画布大小\n" +
                "在画图工具中，如果用户将鼠标移到画布边缘的时候，鼠标指针需要变成可拖动的图标，当按下鼠标左键时，就可以通过鼠标的移动来改变画布的大小。我们在创建主界面的时候，就将画布设计为一个JScrollPane，因此对画布进行拖动，就可以设置画布的大小。在AbstractTool中提供一个dragBorder的方法。\n" +
                ...
                "\t\t}\n" +
                "\t}\n" +
                "这个方法由AbstractTool的mouseDragged方法调用，也就是有鼠标拖动动作时，肯定会调用到这个方法。在改变图像大小之前，需要对获鼠标的当前位置做一些判断，首先获取取鼠标当前的x与y坐标，如果鼠标的类型是属于改变大小类类型，而且鼠标的位置坐标x与y都大于0（见代码中的加粗部分），那就进入改变画布大小的代码段。改变画布大小的方法是先以鼠标当前的x与y坐标创建一张新的图片，并把这张图片的底色设置为白色，再把原来的图片用Graphics的drawImage方法画到现在的画布上。AbstractTool中的mouseDragged方法是对Tool接口定义的mouseDragged的实现。\n" +
                "6.5.2 实现父类的画图方法\n" +
                "当鼠标选择了某种工具，例如直线工具，可以在通过在画板上拖动画出一条直线，当然也可以拖动画出矩形、椭圆形等图形。在AbstractTool中，定义了一个draw方法完成这些工作，但这只是一个空方法，具体的实现由其子类去重写，因为每种工具画出的图形都不一样，所以由其子类去重写。而这个方法由一个createShape方法去调用，在createShape中，主要是把draw方法中一些共同点以同样的方式处理，避免子类的draw方法的代码重复。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\tool\\AbstractTool.java\n" +
                ...
                "\t\t}\n" +
                "\t}\n" +
                "如果鼠标的位置是在画布的范围内，首先将图片重画，这样做的原因是在鼠标松开之前，并没有真正把图形画出来，只是显示这个轨迹，然后再把图片的isSaved属性（是否已经保存）改变为false，最后调用draw方法真正画图。我们要明白何时调用createShape的方法，当鼠标拖动或者松开的时候，就需要调用画图的方法，将图画到界面中，也就是在调用mouseReleased与mouseDragged方法的时候，就需要调用这个createShape的工具方法。\n" +
                "在实现这个工具方法的过程中，我们不知不觉使用了“模板方法（TemplateMethod）”这一种设计模式，模板方法是定义某个操作中的算法结构，而将这个结构中的某一步具体步骤延迟到子类中加载，这个设计模式可以使子类不必去改变整个算法的结构而重新定义该算法的某个具体步骤。在以上的代码中，我们在createShape中定义了具体画图的步骤，但是，以上的代码将draw方法留给了子类去实现。\n" +
                "6.5.3 鼠标移动时改变指针\n" +
                "当鼠标移动到画布边缘时，我们要在这种情况下改变鼠标的指针形状，为鼠标的移动加入监听器，由于是鼠标移动执行的方法，我们需要在实现Tool接口的mouseMoved方法的时候，改变鼠标的指针，鼠标有三种形状类型：Cursor.NW_RESIZE_CURSOR（往右下拖动）、Cursor.W_RESIZE_CURSOR（往右边拖动）、Cursor.S_RESIZE_CURSOR（往下拖动），改变鼠标形状的方法是用这三个类型中的一个去创建一个Cursor，并调用drawSpace中的setCursor设置现在使用的鼠标类型，以下是AbstractTool对该功能的实现。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\tool\\AbstractTool.java\n" +
                "\tpublic void mouseMoved( MouseEvent e) {\n" +
                "\t\t//获取鼠标现在的x与y坐标\n" +
                "\t\tint x = e.getX();\n" +
                ...
                "\t}\n" +
                "6.5.4 记录记录鼠标按下的位置\n" +
                "在使用工具时，不管是画直线或者矩形，总是需要记录鼠标按下时的坐标位置，加上鼠标松开时的坐标位置，才能把一个图形准确地画出来，所以在mousePressed方法中，主要是记录鼠标按下时的位置，以下是AbstractTool的mousePressed方法的实现。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\tool\\AbstractTool.java\n" +
                "public void mousePressed(MouseEvent e) {\n" +
                "\t//如果位置在图片范围内，设置按下的坐标\n" +
                ...
                "}\n" +
                "6.5.5 重绘图片\n" +
                "每次松开鼠标时，可能就是一个画图动作的完成，所以AbstractTool在实现Tool的mouseReleased方法时需要做两步工作，一个是再次调用createShape方法去画图，另外一个是调用drawSpace的repaint方法去刷新，见以下代码。\n" +
                "代码清单：code\\image\\src\\org\\crazyit\\image\\tool\\AbstractTool.java\n" +
                "\tpublic void mouseReleased( MouseEvent e ) {\n" +
                "\t\t//画图\n" +
                ...
                "\t}\n" +
                "到此，AbstractTool都已经实现，AbstractTool实现了Tool接口的全部方法，并提供了一些方法由它的子类去实现，接下面，将实现各个工具的功能。\n" +
                "6.5.6 铅笔工具\n" +
                "铅笔工具，实现的方法主要是在拖动鼠标的时候，每次都以画直线的形式去画下铅笔的轨迹，先看以下代码。\n" +
                ...
                "\t\t}\n" +
                "\t}\n" +
                "首先是用BufferdImage中的getGraphics获取图片的Graphics对象g，由于我们在ImageFrame中保存到一个BufferedImage的对象（BufferImage的子类），因此可以直接获得。如果鼠标的位置是在画布之中（getPressX() > 0 && getPressY() > 0），便把g的color设置为当前工具的颜色，以按下时的坐标与鼠标当前的坐标位置为参数去调用Graphics的drawLine方法画直线，并把按下时的坐标位置设置位当前坐标位置，这样做的原因是想达到铅笔的效果，也就是说每次画直线的起点坐标都是上次的终点坐标，最后调用drawSpace的repaint方法重绘图片。在本章中，铅笔工具对应的是PencilTool类，使用铅笔工具效果如图6.8所示。\n" +
                "\n" +
                "图6.8 使用铅笔工具画图\n" +
                "6.5.7 直线工具、矩形工具、椭圆工具和圆矩形工具\n" +
                "这只个类分别代表直线工具、矩形工具、椭圆工具、圆矩形工具，实现的方法很简单，就是调用Grapchis的drewXXXX方法（drawLine、drawRect、drawOval、drawRoundRect），都重写父类的draw（画图形方法）方法，draw方法最后由createShape方法调用。\n" +
                ...
                "在本章中，直线工具对应的是LineTool类，矩形工具对应的是RectTool类，椭圆形工具对应的是，RoundTool类，圆矩形工具对应的是RoundRectTool类。直线工具、矩形工具、椭圆形工具与圆矩形工具的具体效果如图6.9所示。\n" +
                "\n" +
                "图6.9 直线工具、矩形工具、椭圆形工具与圆矩形工具的效果\n" +
                "6.7 本章小结\n" +
                "本章主要是通过画图工具的基本实现，向读者展未了使用Graphic类的drawLine、drawRect等drawXXXX方法，使用MouseMotionListener、MouseListener等监听器这些普通的类，实现了这些画图的基本功能，加深读者对这些类与方法的理解，学会更好更有技巧地去使用这些swing包中的类。本章中除了介绍一些画图的方法外，我们还使用了模板方法这个设计模式，并将工具抽象成一个接口，使得我们编写的程序耦合性更低，向读者展现了面向对象的多态、继承与封装的特性。\n')");
        db.execSQL("insert into book_chapter(book_english_name,chapter_name,chapter_id,content)values('crazy_java','第7章 单机连连看','7','7.1 连连看游戏简介\n" +
                ...
                "注：以上两个属性，同样加到Piece对象的构造器中。\n" +
                "7.3.4 创建游戏处理类\n" +
                "新建一个GameService的接口，用于定义游戏逻辑的接口方法，再为这个接口新增一个实现类GameServiceImpl，将GameService接口设置到GamePanel这个视图组件中，这们就可以达到逻辑与视图分离了。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\GameService.java\n" +
                "public interface GameService {\n" +
                "\t//定义一个接口方法, 用于返回一个二维数组\n" +
                "\tPiece[][] getPieces();\n" +
                "}\n" +
                "以后我们需要实现某些游戏逻辑，可以在这个接口里面定义方法，并由GameServiceImpl去实现了。这样可以简化视图的代码，也遵循了单一职责的原则。现在我们为GamePanel类加入一个GameService的属性，并为它的构造器中加入设置GameService的代码，用于设置GameService，使得在GamePanel中可以拿到GameService对象，代码如下。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\view\\GamePanel.java\n" +
                "\tprivate GameService gameService; \n" +
                "\tpublic GamePanel(GameService gameService) {\n" +
                "\t\t//省略其他设值的代码\n" +
                "\t}\n" +
                "现在，我们可以去修改GamePanel中的paint方法，代表游戏区域的二维数组Piece[][]的创建与变化，我们可以放到GameService的实现类中去，在GameService中提供一些接口方法，GamePanel通过这些方法去获取二维数组，而如何去创建，设置这个二维组数里面的值，GamePanel不再需要去理会这些过程。如果GamePanel类的paint方法中可以这样得到Piece数组。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\view\\GamePanel.java\n" +
                "\tpublic void paint(Graphics g) {\n" +
                "\t\t//使用GameService来获取Piece[][]二维数组\n" +
                ...
                "首先我们必须准备好游戏中所需要的图片，本例中准备了23张内容不同的图片用于作为图片方块。\n" +
                "注：这23张图片的长和宽必须一样，使界面好看，也符合连连看的规则。\n" +
                "接下来，我们去创建用于读取图片和处理图片的ImageUtil工具类，在为这个工具类添加方法前，我们必须要明确这个类的作用，读取或者处理图片的一些公共方法可以放到这里，做成静态方法给外部使用。现在我们新建一个读取图片的方法，读取某个文件夹下面的符合后缀的图片，我们把方块的图片独立放到一个文件夹，再使用程序读取它们，并封装成一个集合返回，实现代码如下。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\utils\\ImageUtil.java\n" +
                "\t//用于获取某个文件夹下面的所有图片\n" +
                "\tpublic static List<BufferedImage> getImages(File folder, String subfix) throws IOException {\n" +
                "\t\t//从目标文件夹中获取文件列表\n" +
                ...
                "\t\t\tif (file.getName().endsWith(subfix)) result.add(ImageIO.read(file));\n" +
                "\t\t}\n" +
                "\t\treturn result;\n" +
                "\t}\n" +
                ...
                "\t\t\t\t//当源图片集合的size为0时，会抛出数组越界的异常，直接返回结果集\n" +
                "\t\t\t\treturn result;\n" +
                "\t\t\t}\n" +
                "\t\t}\n" +
                "\t\treturn result;\n" +
                "\t}\n" +
                "这样，我们就可以从文件夹中读取图片作为我们游戏的方块了，但是，前面说到，在游戏中每一种图片在游戏区域中出现的次数必须为双数，游戏才可以结束，因此，我们需要在读取图片的时候做一些相关处理。例如，我们的游戏区域（有图片的区域）为10乘10大小，即横10个方块，竖10个方块，那么我们将要读取100张图片作为方块，由于每种图片必须为双数，因此我们将100除以2，也就是需要随机拿50张图片即可，再将这50张图片乘以2，即是我们所要的100张图片，也可以保证每种图片的数量是双数。现在，我们需要有一个方法去打乱一个集合里面的元素，就好像这样：\n" +
                "\uF071\t源：\t\t0，1，2，3，4，5\n" +
                "\uF071\t结果：\t2，1，5，0，4，3\n" +
                ...
                "\t\treturn result;\n" +
                "\t}\n" +
                "我们在这里先完成了一个工具类用于处理图片，那么在创建游戏区域的时候，我们可以不用在GameService这个逻辑类里面实现图片读取和随机打乱的功能，只需要调用这一个工具类就可以达到同种图片为双数，并随机打乱图片的功能。\n" +
                "7.3.6 创建游戏区域图片数组\n" +
                "在画游戏区域之前，我们必须要为游戏区域数组进行赋值，将游戏区域看作一个二维数组，那么这个数组的每个值的变化，将会影响到游戏区域的展现，因此这个二维数组对我们的游戏尤为重要。\n" +
                "我们现在为GameService接口提供一个start方法，用于执行游戏开始时的一些动作，例如初始化游戏区域，重新计时，重新计分等，当玩家点击了开始时，我们就要调用这个start方法开始游戏，我们可以为开始按钮创建鼠标监听器。当然，我们这里先讲创建游戏区域，GameServiceImpl代码如下。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\tpublic void start() {\n" +
                "\t\t//创建一个有100个方块的游戏区域\n" +
                "\t\tthis.pieces = new Piece[10][10];\n" +
                "\t\t//获取游戏图片，数量为Piece数组一维的长度乘以二维的长度\n" +
                ...
                "\t\t\t\t//构造一个Piece对象，以一维的值乘以图片的宽作为该对象的开始x坐标\n" +
                "\t\t\t\t//以二维的值乘以图片的高作为该对象的开始y坐标\n" +
                ...
                "\n" +
                "图7.3 开始坐标与结束坐标的计算\n" +
                "如图7.3，就清楚上面的代码为什么要这样实现了，现在，我们的游戏区域数组已经初始化好了，剩下的就是如果将该数组“画”到GamePanel中了。\n" +
                "7.3.7 根据数组画游戏区域\n" +
                "在7.3.6中已经创建好的游戏区域数组，现在去实现GamePanel的paint方法，用于将游戏区域数组“画”到GamePanel中，可以在对Piece数组进行循环的过程中，遇到某个非空Piece，即“画”到游戏区域中，具体的代码如下。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\view\\GamePanel.java\n" +
                "\t\t//调用Graphics类的drawImage方法画图，从piece的开始坐标开始画\n" +
                "\t\tg.drawImage(piece.getImage(), piece.getBeginX(),piece.getBeginY(), null);\n" +
                "这样，就可以进这个GamePanel进行绘制了，修改GameServiceImpl类中的start方法，设置第一个方块出现的x坐标和y坐标：\n" +
                "Piece piece = new Piece(i * imageWidth + 30, j * imageHeight + 30, i, j, playImages.get(index));\n" +
                "注意上面的粗体字，代表从第一个方块开始，x和y值分别加30，即向游戏区域的右下角各移动30。重新运行，并点击开始，效果图7.4所示。\n" +
                "\n" +
                "图7.4 将二维数组“画”到游戏区\n" +
                "7.3.8 随机初始化游戏\n" +
                "我们定义一个AbstractBoard对象，主要用于创建游戏图片，我们可以将以上创建游戏区域的代码放置到该对象中，统一由该对象负责处理游戏图片的创建。代码如下：\n" +
                ...
                "\t\t//省略以下代码\n" +
                "写一个SquareBoard类去继承AbstractBoard，并实现它的createPieces方法。\n" +
                ...
                "\t\t\t\tnotNullPieces.add(piece); //添加到非空Piece对象的集合中\n" +
                "\t\t\t}\n" +
                "\t\t}\n" +
                "\t\treturn notNullPieces;\n" +
                "\t}\n" +
                "}\n" +
                "这样，如果还需要构造不同的游戏数组，我们可以去继承AbstractBoard类，并实现createPieces即可。如果现在还需要构建新的游戏数组，那么我们再写一个子类SimpleBoard去继承AbstractBoard并实现createPieces方法，以下是SimpleBoard的主体代码。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\SimpleBoard.java\n" +
                "\t\tList<Piece> notNullPieces = new ArrayList<Piece>();\n" +
                "\t\tfor (int i = 0; i < pieces.length; i++) {\n" +
                "\t\t\tfor (int j = 0; j < pieces[i].length; j++) {\n" +
                "\t\t\t\t//加入判断, 符合一定条件才去构造Piece对象, 并加到集合中\n" +
                "\t\t\t\tif (i % 2 == 0) {//如果x能被2整除, 即单数列不会创建方块\n" +
                "\t\t\t\t\t//先构造一个Piece对象, 只设置数组中的位置为i, j，其他值不设置\n" +
                "\t\t\t\t\tPiece piece = new Piece(i, j);\n" +
                "\t\t\t\t\t//添加到非空Piece对象的集合中\n" +
                ...
                "\t\t\t}\n" +
                "\t\t}\n" +
                "createPieces方法中直接回返结果集合即可。现在我们编写了SimpleBoard与SquareBoard两个对象，如果需要随机创建这两个实例，可以通过Random来实现。游戏每次start的时候，就会创建出不同的游戏数组了，效果如图7.5所示。\n" +
                "\n" +
                "图7.5 随机数组\n" +
                "其实，上面SquareBoard与SimpleBoard继承于AbstractBoard，使用了其中的一种设计模式：模板方法。模板方法的概念：定义一个操作中的算法的结构，而将一些步骤延迟到子类中。模板方法使得子类可以不改变一个算法的结构即可重定义该算法的某些特定步骤（摘自《设计模式》）。上面的SquareBoard与SimpleBoard，继承了AbstractBoard，AbstractBoard中定义了图片生成的算法，它的两个子类就是实现父类定义的结构的某一个步骤。\n" +
                "7.4 实现连接程序\n" +
                "在7.3中，我们已经可以创造一个随机的数组，这一节，我们开始实现连接的程序。所谓连接，就是将游戏区域内的两个相同图片按照一定的规则连接起来并消除，在这里，游戏区域中已经保存了多个Piece对象，我们在点击连接时，只需要将这些Piece对象消除就可以实现连接。\n" +
                "7.4.1 图片选择\n" +
                "在图片连接前，我们需要选择一张图片，并标示它已经被选择了。只需要在为鼠标的点击加入事件监听器。当鼠标在游戏区域进行点击后，根据点击的坐标得到具体的某个Piece对象。新建一个findPiece方法，根据鼠标的x坐标和y坐标（即鼠标点击的某个点）获取点击的那个Piece对象。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\tpublic Piece findPiece(int mouseX, int mouseY) {\n" +
                "\t\t//由于是在本类的board中找Piece对象, 如果board为空, 即游戏区域中没有方块\n" +
                ...
                "\t\t//如果鼠标点击的地方比游戏区域中第一张图片的开始x坐标和开始y坐标要小, 即没有找到方块\n" +
                "\t\tif (relativeX < 0 || relativeY < 0) {  return null; }\n" +
                "\t\t//获取relativeX坐标在游戏区域数组中的一维值, 第二个参数为每张图片的宽\n" +
                "\t\tint indexX = getIndex(relativeX, this.board.getCommonImageWidth());\n" +
                "\t\t//获取relativeY坐标在游戏区域数组中的二维值, 第二个参数为每张图片的高\n" +
                "\t\tint indexY = getIndex(relativeY, this.board.getCommonImageHeight());\n" +
                "\t\t//返回本对象中游戏区域数组的某个值\n" +
                "\t\treturn this.pieces[indexX][indexY];\n" +
                "\t}\n" +
                "通过findPiece方法，我们可以通过鼠标定位到数组中的某个Piece对象了，接着，还要处理当选择了某一个Piece对象时，需要标识它为已经被选中的状态，这时候需要告诉鼠标监听器，鼠标选择了哪一个Piece，再告诉GamePanel当前选择了哪一个Piece，让GamePanel去重新绘画。这里需要注意的是GamePanel只允许出现一个被选中的Piece对象，当选择第二个的时候，如果可以相连，那么将设置selectPiece为null，如果不可以相连，那么selectPiece为选择的第二个。运行游戏并对图片进行选择效果如图7.6所示。\n" +
                "\n" +
                "图7.6 方块被选中的效果\n" +
                "图7.6中可以看到，游戏区域中最左上角（0，0）的那个方块已经被选中。我们需要为GamePanel加入一个集合来保存已经选择的Piece对象，当选择了一个Piece对象后，将其加入到集合中。每选择一次，都需要对集合进行判断，如果集合的大小为0，则直接加入到集合中，如果集合大小为1，就表示之前已经选择了一个Piece，就可以判断是否可以消除。\n" +
                "7.4.2 创建连接的相关对象\n" +
                "在实现连接功能前，我们还需要准备一些对象，用于封装在连接过程中会用到的对象，例如，连接是由某些连接点构成的，这里需要一个Point对象，连接可以看作一个LinkInfo对象，即表示连接信息。LinkInfo里有一个集合属性，用于存放每一个Point。具体代码如下：\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\commons\\Point.java\n" +
                "public class Point { \n" +
                "\tprivate int x; //记录这个点对象的x坐标\n" +
                "\tprivate int y; //记录这个点对象的y坐标\n" +
                ...
                "\t//下面省略setter和getter方法\n" +
                "}\n" +
                "上面新建一个Point类，用于保存点的x和y坐标，注意，该类重写的equals方法，用于判断两个Point对象是否为同一个点，判断标准为两个点的x坐标与y坐标是否一致。\n" +
                ...
                "\t}\n" +
                "}\n" +
                "该类只提供三个构造器和一个返回存放连接点集合的方法，构造器用于创造连接点信息对象，如果两点可以直接连接，则调用两个参数的构造器，两个点之间需要有一个转折点才可以相连的话，需要调用三个参数的构造器，两个点之间有两个转折点的话，就需要调用四个参数的构造器。图7.7解析这三个构造器的作用。\n" +
                "\n" +
                "图7.7 LinkInfo类的描述\n" +
                "如图7.7，我下面的编码就是按照这三种方式来实现，先判断能不能直接相连，再判断一个转折点的情况，最后为两个转折点的情况。我们还需要GameListener类根据GameService的link方法返回的LinkInfo对象进行处理，让GamePanel对象进行重绘。\n" +
                "为GameService接口加新方法，让GameListener调用。在GameService接口中添加link方法，让GameServiceImpl去实现它即可，link方法的参数就是两个Piece对象，返回值就是我们上面所定义的LinkInfo对象了，表示可否根据两个Piece对象返回一个LinkInfo对象，如果返回null，则表示两个对象不可连接。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\GameService.java\n" +
                "\t//连接两个Piece对象, 可以连接, 返回LinkInfo对象\n" +
                "\tLinkInfo link(Piece p1, Piece p2);\n" +
                "我们需要对游戏的游戏区域进行分析，连接的情况在图7.7已经描述了，那么接下来就是两个Piece在游戏区域中的关系了，具体可分为：p1与p2在同一行（indexY值相同），p1与p2在同一列（indexX值相同），p2在p1的右上角（p2的indexX > p1的indexX，p2的indexY < p1的indexY），p2在p1的右下角（p2的indexX > p1的indexX，p2的indexY > p1的indexY），这里为什么不讲p2在p1的左上角和p2在p1的左下角呢，这种情况，我们可以重新执行link方法，将p1和p2两个参数的位置互换即可实现。图7.8讲解了这几种位置的情况。\n" +
                "\n" +
                "图7.8 两个Piece对象的位置关系\n" +
                "注：如果p2在p1的左上角和左下角，也就是p1在p2的右上角和右下角，也在上面的情况中。\n" +
                "现在了解了这几种情况后，下面就可以着手写一些工具方法，用于处理在上面这几个情况时会用到的一些公用方法。\n" +
                "7.4.3 准备获取通道的工具方法\n" +
                "现在准备几个工具方法，在实现时我们需要用到的。我们需要获取一个Piece向上的通道，向下的通道，向左的通道，向右的通道，什么叫通道呢？如图7.9所示。\n" +
                "\n" +
                "图7.9 通道的描述\n" +
                ...
                "7.4.4 没有转折点的横向连接\n" +
                "现在让我们先实现最简单的横向连接（没有转折点），可以实现link方法。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\t//实现接口的link方法\n" +
                "\tpublic LinkInfo link(Piece p1, Piece p2) {\n" +
                "\t\t//两个Piece是同一个, 即在游戏区域中选择了同一个Piece, 返回null\n" +
                "\t\tif (p1.equals(p2)) return null;\n" +
                "\t\t//如果p1的图片与p2的图片不相同, 则返回null\n" +
                "\t\tif (!p1.isSameImage(p2)) return null;\t\n" +
                "}\n" +
                "上面的代码，如果在游戏区域中选择了同一个Piece，则直接返回null，如果p1与p2的图片不相同，表示它们不可连，直接返回。再去新增两个工具方法，用于判断一行（一列）的两个点是否有障碍。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                ...
                "\t\treturn false;\n" +
                "\t}\n" +
                "有了这两个方法之后，我们可以判断两个在同一行（同一列）的点之间是否可连了。下面实现同一行（同一列）可以连的情况，这里是指没有转折点的情况，修改link方法。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\t//实现接口的link方法\n" +
                "\tpublic LinkInfo link(Piece p1, Piece p2) {\n" +
                "\t\t//两个Piece是同一个, 即在游戏区域中选择了同一个Piece, 返回null\n" +
                ...
                "图7.10 实现横向相连\n" +
                "7.4.5 没有转折点的纵向连接\n" +
                "下面实现纵向连接，和横向连接一样，调用工具方法isYBlock判断是否可以纵向连接即可，为link方法加入如下代码。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\t\t//如果两个Piece在同一列\n" +
                ...
                "\t\t\t}\n" +
                "\t\t}\n" +
                "如果两个Piece在同一列，并且它们可以直接纵向相连（没有转折点），就返回LinkInfo，效果如图7.11所示。\n" +
                "\n" +
                "图7.11 实现纵向相连\n" +
                "上面实现了没有转折点的横向连接和没有转折点的纵向连接，如果在同一行或者同一列的两个Piece不能直接连接，即它们肯定会有两个转折点，下面将会实现。\n" +
                "7.4.6 一个转折点的连接\n" +
                "现在我们先看回图7.8，第二种情况，表示有一个转折点，我们需要找到这个转折点，新增一个遍历两个通道，获取它们交点的工具方法。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\t//遍历两个通道, 获取它们的交点\n" +
                ...
                "\t\t\t\t\treturn temp1; //这个交点就是我们所需要的转折点\n" +
                "\t\t\t\t}\n" +
                "\t\t\t}\n" +
                "\t\t}\n" +
                "\t\treturn null;\n" +
                "\t}\n" +
                "这样可以获取两个通道的交点，再新增一个工具方法，用于获取两个Point之间的转折点（一个Point）。代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                ...
                "\t\treturn null;\n" +
                "\t}\n" +
                "以上的代码获得两个Piece之间的转折点，如图7.12所示。\n" +
                "\n" +
                "图7.12 一个转折点，p2在p1的右下角\n" +
                "图7.12的情况，我们需要获取p1向右的通道，再获取p2向上的通道，再判断两个通道间是否有交点，并返回该点即可。当p2在p1的右上角的情况如图7.13所示。\n" +
                "\n" +
                "图7.13 一个转折点，p2在p1的右上角\n" +
                "p2在p1的右上角，即获取p1的上右通道，p2的向下通道，再获取它们的交点即可。在GameServiceImpl的link方法中加入判断代码，到现在link方法的代码如下。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "\t//实现接口的link方法\n" +
                "\tpublic LinkInfo link(Piece p1, Piece p2) {\n" +
                ...
                "\t}\n" +
                "7.4.7 两个转折点的连接\n" +
                "上一小节讲了两个Point之间一个转折点的情况，这一节，将实现两个Point通过两个转折点进行相连，两个转折点的情况比较多，可以总结为以下几种：\n" +
                "\uF071在同一行，不能直接相连，就必须有两个转折点，分向上与向下两种连接情况。\n" +
                "\uF071在同一列，不能直接相连，也必须有两个转折点，分向左与向右两种连接情况。\n" +
                "\uF071p2在p1的右下角，这里就有六种转折情况。\n" +
                "\uF071p2在p1的右上角，同样地也有六种转折情况。\n" +
                "我们一种一种来解决，先解决在同一行有两个转折点的情况。请看图7.14，说明它们是如何相连的。\n" +
                "\n" +
                "图7.14 两个转折点，p1与p2在同一行\n" +
                "图7.14可以看到，它们p1与p2相连，可以在上面连，也可以在下面连，这两种情况都代表它们可以相连，我们先把这两种情况都加入结果中，到最后再去计算最近的距离。实现时我们可以先构建一个Map，Map的key为第一个转折点，map的value为第二个转折点，如map的size不止1的话，证明这两个Point有多种连接途径，我们先返回第一个连接途径，最后再计算最小的连接方式。为link方法添加如下代码。\n" +
                "代码清单：code\\linkgame\\src\\org\\crazyit\\linkgame\\service\\impl\\GameServiceImpl.java\n" +
                "        //该map的key存放第一个转折点, value存放第二个转折点, map的size说明有多少个可以连的途径\n" +
                "        Map<Point, Point> turns = getLinkPoints(p1Point, p2Point, pieceWidth, pieceHeight);\n" +
                "        if (turns.size() != 0) {\n" +
                "            for (Object turn : turns.keySet()) {\n" +
                "                //遍历该map, 暂时只返回map中第一个元素\n" +
                "                //获取map中第一个元素的key值, 即第一个转折点\n" +
                "                Point point1 = (Point)turn;\n" +
                "                //获取map中第一个元素的value值, 即第二个转折点\n" +
                "                Point point2 = turns.get(point1);\n" +
                "                return new LinkInfo(p1Point, point1, point2, p2Point);\n" +
                "            }\n" +
                "        }\n" +
                "上面的代码有点难以理解，可以看图7.15。\n" +
                "\n" +
                "图7.15 横向两个转折点的代码解释\n" +
                "图7.15中的几条黑线，如果这几条黑线间没有障碍，则最终会加到结果的map中。效果如图7.16。\n" +
                "\n" +
                "图7.16 横向两个转折点的相连\n" +
                "\n" +
                "图7.27 最终的游戏效果\n" +
                "7.6 本章小结\n" +
                "本章主要讲述如何实现一个简单的单机版连连看，详细介绍了如何建立界面，如何将游戏的区域抽象成一个二维数组，并对该二维数组进行操作，实现了随机游戏数组，实现游戏的相连，游戏的计分功能与计时等功能。重点介绍了如何在重构代码的过程中优化代码并慢慢的改善既有设计，将游戏的部件或者各部件的职能抽象成对象。在本章中代码还可以进行重构优化，例如将每个图形界面组件做成单独的类，再通过工厂模式或者Java的反射去获取这些类，用于去创建游戏的界面。')");
    }
